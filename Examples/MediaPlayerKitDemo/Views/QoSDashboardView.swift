        ...
        return List {
            if data.isEmpty {
                ...
                Section(header: Text("播放质量 · 会话累计")) {
                    row("有效播放时长", number(session["play_ms"], unit: " s", scale: 0.001))
                    row("卡顿总时长", number(session["stalledTotalDuration"], unit: " s", scale: 0.001))
                    row("卡顿次数", number(session["stalledCount"]))
                    row("卡顿占比", number(session["stall_ratio"], unit: "%", scale: 100))
                    row("当前尝试首帧耗时", number(data["first_frame_time"], unit: " ms"))
                    row("当前内核创建耗时", number(data["create_ms"], unit: " ms"))
                }
                Section(header: Text("切源与恢复 · 会话累计")) {
                    row("播放尝试", number(data["attempts"]))
                    row("切换播放源", number(data["source_switches"]))
                    row("同源切换内核", number(data["engine_switches"]))
                    row("尝试失败", number(data["errors"]))
                    row("恢复成功", number(data["recover_ok"]))
                    row("恢复失败", number(data["recover_fail"]))
                    row("恢复取消", number(data["recover_cancel"]))
                    row("最近恢复耗时", number(data["recover_ms"], unit: " ms"))
                }
                Section(header: Text("最近内核采样 · 非逐请求统计")) {
                    row("显示帧率", number(metrics["fps"], unit: " fps"))
                    row("视频标称帧率", number(metrics["frame_rate"], unit: " fps"))
                    row("观测吞吐率", number(metrics["bandwidth"], unit: " bps"))
                    row("网络下载速度", number(metrics["net_speed"], unit: " B/s"))
                    row("本周期网络字节", number(metrics["net_bytes"], unit: " B"))
                    row("本周期媒体请求", number(metrics["media_requests"]))
